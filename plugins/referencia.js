...
const handler = async (m, { command, args, text, usedPrefix }) => {
    ...
    try {
        if (command.toLowerCase() !== "soundcloudr") {
            ...
            let listSections = [];
            for (let index = 0; index < urls.length && index < 5; index++) { // Limitar a 5 opciones
                        ...
                        rows: [
                            {
                                ...
                                rowId: `${usedPrefix}soundcloudr ${urls[index]}`
                            }
                        ]
                    });
                }
            ...
            const imageUrl = "https://i.pinimg.com/736x/1c/b9/dc/1cb9dce731c1544b0bd018b02567fd1f.jpg";
            const { imageMessage } = await prepareWAMessageMedia({
                image: { url: imageUrl }
            }, { upload: conn.waUploadToServer });

            return await conn.sendMessage(m.chat, {
                text: `${htki} *𝙍𝙀𝙎𝙐𝙇𝙏𝘼𝘿𝙊𝙎* ${htka}\n\n𝘽𝙪𝙨𝙦𝙪𝙚𝙙𝙖 𝙙𝙚: ${text}`,
                footer: 'Selecciona una opción',
                title: '𝗕 𝗨 𝗦 𝗖 𝗔 𝗥',
                buttonText: 'Selecciona aquí',
